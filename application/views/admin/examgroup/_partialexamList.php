...
if ($this->rbac->hasPrivilege('exam_assign_view_student', 'can_view')) {
   ...
   title = "<?php echo $this->lang->line('assign / view'); ?>" class="btn btn-default btn-xs assignStudent"  id="load" data-examid="<?php echo $exam_value->id; ?>" ><i class="fa fa-tag"></i></button>
    ...
    if ($this->rbac->hasPrivilege('exam_subject', 'can_view')) {
        ?>
		 	 <button class="btn btn-default btn-xs" id="subjectModalButton" data-toggle="tooltip"  data-class_batch_id="<?php echo $exam_value->class_batch_id; ?>" data-exam_id="<?php echo $exam_value->id; ?>"  title="<?php echo $this->lang->line('exam') . " " . $this->lang->line('subjects') ?>"><i class="fa fa-book" aria-hidden="true"></i></button>
		 	<?php
}
